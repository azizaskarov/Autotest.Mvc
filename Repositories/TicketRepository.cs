...
public class TicketRepository
{
    ...
    private void CreateTicketTable()
    {
        ...
        command.CommandText = "CREATE TABLE IF NOT EXISTS tickets(id INTEGER PRIMARY KEY AUTOINCREMENT, user_id TEXT, questions_count INTEGER, start_index INTEGER, current_question_index INTEGER, date BIGINT)";
    ...
    {
        var command = _connection.CreateCommand();
        command.CommandText = "INSERT INTO tickets(user_id, questions_count, start_index, current_question_index, date) VALUES(@user_id, @questions_count, @start_index, @current_question_index, @date)";
    ...
    public List<Ticket> GetTicketList(string userId)
    {
        ...
        while (reader.Read())
        {
            ...
            ticket.Answers = GetTicketAnswers(ticket.Id);
            tickets.Add(ticket);
        }
